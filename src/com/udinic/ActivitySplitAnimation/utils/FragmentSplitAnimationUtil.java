...
import com.udinic.ActivitySplitAnimation.fragment.BaseFragment;

public class FragmentSplitAnimationUtil {
    ...
    /**
     * Show next fragment with a Split animation
     *
     ...
     */
    public static void showFragment(BaseFragment currentFragment, BaseFragment nextFragment, int splitYCoord) {

        // Preparing the bitmaps that we need to show
        prepare(currentFragment, splitYCoord);
        currentFragment.showFragment(nextFragment);
    }

    /**
     * Show next fragment with a Split animation right in the middle of the Activity
     *
     * @param currActivity The current Fragment
     * @param nextFragment  The next fragment
     */
    public static void showFragment(BaseFragment currentFragment, BaseFragment nextFragment) {
    	showFragment(currentFragment,nextFragment, -1);
    }

    /**
     * Preparing the graphics on the base Activity.
     * Should be called on the next Fragment on Fragment#onShowSelf()
     *
     * @param baseActivity base activity of fragment
     */
    public static void prepareAnimation(final Activity baseActivity) {
        mTopImage = createImageView(baseActivity, mBitmap, mLoc1);
        mBottomImage = createImageView(baseActivity, mBitmap, mLoc2);
    }

    /**
     * Start the animation the reveals the next fragment
     *
     * @param baseActivity base activity of fragment
     * @param duration The duration of the animation
     * @param interpolator The interpulator to use for the animation. null for no interpulation.
     */
    public static void animate(final Activity baseActivity, final int duration, final TimeInterpolator interpolator) {
    ...
    /**
     * Start the animation that reveals the next Fragment
     * 
     *
     * @param baseActivity base activity of fragment
     * @param duration The duration of the animation
     */
    public static void animate(final Activity baseActivity, final int duration) {
        animate(baseActivity, duration, new DecelerateInterpolator());
    }
     ...
     *
     * @param baseFragment the current fragment from where we start the new one
     * @param splitYCoord  The Y coordinate where we want to split the activity. -1 will split the activity equally
     */
    private static void prepare(BaseFragment baseFragment, int splitYCoord) {

        // Get the content of the activity and put in a bitmap
        View root = baseFragment.getActivity().getWindow().getDecorView().findViewById(android.R.id.content);
    ...
    /**
     * Creating the an image, containing one part of the animation on the next fragment
     *
     * @param baseActivity The base activity
     * @param bmp          The Bitmap of the part we want to add to the next fragment
     * @param loc          The location this part should be on the screen
     * @return
     */
    private static ImageView createImageView(Activity baseActivity, Bitmap bmp, int loc[]) {
    	MyImageView imageView = new MyImageView(baseActivity);
        ...
        baseActivity.getWindowManager().addView(imageView, windowParams);
